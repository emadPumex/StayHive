...
const PropertyCard = ({property}) => {
    ...
    return (
        <Link to={`/property/${id}`} className="block group">
            <div
                className="bg-[#0F1117] border border-[#1A1D26] rounded-2xl overflow-hidden flex flex-col h-full hover:border-[#2A2D38] transition-colors duration-300">
            ...
            <span className="flex items-center gap-1 text-xs text-[#8A8FA8]">
              <MapPin className="w-3.5 h-3.5 text-[#C8FB4C] shrink-0"/>
                ...
                <Star className="w-3 h-3 fill-[#C8FB4C]"/> {rating}%
                    ...
                    {/* Specs */}
                    <div
                        className="grid grid-cols-3 gap-1 py-2 border-t border-b border-[#1A1D26] text-[11px] text-[#8A8FA8] font-medium">
                    ...
                    <div className="flex flex-wrap gap-1">
                        {amenities?.slice(0, 3).map(a => (
                            <span key={a}
                                  className="text-[10px] font-semibold bg-[#1A1D26] text-[#8A8FA8] px-2 py-0.5 rounded-md">
                {a}
              </span>
                        ))}
                        {amenities?.length > 3 && (
                            <span
                                className="text-[10px] text-[#3A3D48] px-1.5 py-0.5">+{amenities.length - 3} more</span>
                        )}
                    </div>
