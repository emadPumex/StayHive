...
const INITIAL_FILTERS = {
  search: '', country: '', market: '', propertyType: '', roomType: '',
  minPrice: '', maxPrice: '', accommodates: '', bedrooms: '', bathrooms: '',
  amenities: [], isSuperhost: false, minRating: '', available30: false,
  cancellationPolicy: '', page: 0, limit: 6,
};

const PropertyListingPage = () => {
  const [filters, setFilters]               = useState(INITIAL_FILTERS);
  const [listingsData, setListingsData]     = useState({ content: [], totalPages: 0, totalElements: 0, first: true, last: true });
  ...
  const fetchListings = useCallback(async (f) => {
    ...
    try { setListingsData(await getListings(f)); }
  ...
  const handleSearch       = ({ search, country, market }) => setFilters(p => ({ ...p, search, country, market, page: 0 }));
  ...
  const activeCount = [
    filters.country, filters.market, filters.propertyType, filters.roomType,
    filters.minPrice, filters.maxPrice, filters.accommodates, filters.bedrooms,
    filters.bathrooms, filters.isSuperhost, filters.minRating, filters.available30,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          >
            ...
            Filters
            {activeCount > 0 && (
                <span className="bg-[#0F1117] text-[#C8FB4C] font-extrabold px-1.5 py-0.5 text-[9px] rounded-full">
              {activeCount}
            </span>
            )}
          </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="hidden lg:block lg:col-span-1">
            <FilterSidebar filters={filters} onChangeFilters={handleFilterChange} onClearFilters={handleClearAll} />
          </div>
          ...
          <div className="lg:col-span-3 flex flex-col min-h-[50vh]">
            ...
            {/* Results bar */}
            <div className="flex items-center justify-between mb-4 text-xs font-bold text-[#3A3D48] uppercase tracking-wider px-1">
              <span>{listingsData.totalElements} stays found</span>
              {hasActive && (
                  <button onClick={handleClearAll} className="text-[#C8FB4C] hover:opacity-75 normal-case font-bold cursor-pointer transition-opacity">
                    Reset filters
                  </button>
              )}
            </div>
            ...
            {isLoading ? (
                ...
                <div className="flex-grow flex flex-col items-center justify-center py-12 text-center bg-[#0F1117] border border-[#1A1D26] rounded-2xl p-6">
                ...
                <div className="flex-grow flex flex-col items-center justify-center py-12 text-center bg-[#0F1117] border border-[#1A1D26] rounded-2xl p-6">
                  ...
                  <p className="text-xs text-[#8A8FA8] mb-5 max-w-xs">No properties matched your filters. Try clearing some selections.</p>
                  {hasActive && (
                      <button onClick={handleClearAll} className="px-4 py-2 bg-[#C8FB4C] text-[#0F1117] font-bold rounded-xl text-xs hover:opacity-90 transition-opacity cursor-pointer">
                        Clear all filters
                      </button>
                  )}
                </div>
