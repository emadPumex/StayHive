...
const PropertyDetailsPage = () => {
  ...
  const { name, propertyType, roomType, price, accommodates, bedrooms, beds, bathrooms, amenities, reviewScores, host, address, images, summary, reviews = [] } = property;

  const rating      = reviewScores?.reviewScoresRating;
  const isSuperhost = host?.hostIsSuperhost;
  const mainImage   = images?.pictureUrl || 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?auto=format&fit=crop&w=800&q=80';
  const gridImages  = [
    mainImage,
  ...
  return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-20">
        ...
        <div className="flex items-center justify-between mb-5">
          <Link to="/properties" className="inline-flex items-center gap-2 text-sm font-bold text-[#8A8FA8] hover:text-[#C8FB4C] transition-colors">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-[#FAFAF8] tracking-tight leading-tight" style={{ fontFamily: "'Outfit', sans-serif" }}>
            {name}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 rounded-2xl overflow-hidden mb-8">
          <div className="md:col-span-2 aspect-[4/3] md:aspect-auto md:h-[400px] overflow-hidden bg-[#1A1D26]">
            <img src={gridImages[0]} alt={name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
          </div>
          <div className="hidden md:grid md:col-span-2 grid-cols-2 gap-2 h-[400px]">
            {gridImages.slice(1, 5).map((url, i) => (
                <div key={i} className="overflow-hidden bg-[#1A1D26]">
                  <img src={url} alt={`${name} ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#0F1117] border border-[#1A1D26] p-6 rounded-2xl space-y-3">
              ...
              <p className="text-sm text-[#3A3D48] leading-relaxed">
                Configured as <span className="font-semibold text-[#8A8FA8]">{roomType}</span>. Ideal for tourists, business nomads, and luxury travelers.
              </p>
            </div>
            ...
            {host && (
                <div className="bg-[#0F1117] border border-[#1A1D26] p-6 rounded-2xl flex flex-col sm:flex-row items-center sm:items-start gap-5">
                  <img src={host.hostThumbnailUrl} alt={host.hostName} className="w-14 h-14 rounded-full object-cover border-2 border-[#2A2D38] shrink-0" />
                  <div className="space-y-2 text-center sm:text-left">
                    ...
                    <p className="text-xs text-[#3A3D48] leading-relaxed">
                      StayHive verified partners complete regular quality, internet bandwidth, and sanitization inspections for a premium stay.
                    </p>
                  </div>
                </div>
            )}
            ...
            <div className="bg-[#0F1117] border border-[#1A1D26] p-6 rounded-2xl space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#1A1D26]">
                ...
                {rating && reviews.length > 0 && (
                    <div className="flex items-center gap-3 bg-[#1A1D26] border border-[#2A2D38] px-4 py-2 rounded-xl self-start">
                      <div className="flex items-center gap-1 text-[#C8FB4C]">
                        <Star className="w-4 h-4 fill-[#C8FB4C]" />
                        <span className="text-sm font-black text-[#FAFAF8]">{(rating / 20).toFixed(1)}</span>
                      </div>
                      <div className="w-px h-4 bg-[#2A2D38]" />
                      <span className="text-[11px] font-bold text-[#3A3D48] uppercase tracking-wide">Excellent</span>
                    </div>
                )}
              </div>
              ...
              {reviews.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {reviews.map((review, i) => (
                        <div key={i} className="p-4 bg-[#1A1D26] border border-[#2A2D38] rounded-xl space-y-3">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 bg-[#C8FB4C] text-[#0F1117] text-xs font-black rounded-xl flex items-center justify-center shrink-0">
                          ...
                          <div className="flex items-center gap-0.5">
                            {Array.from({ length: 5 }).map((_, si) => (
                                <Star key={si} className={`w-3 h-3 ${si < review.rating ? 'text-[#C8FB4C] fill-[#C8FB4C]' : 'text-[#2A2D38]'}`} />
                            ))}
                          </div>
                  ...
                  <div className="flex flex-col items-center justify-center text-center p-8 border border-dashed border-[#2A2D38] rounded-xl gap-3">
                    <div className="p-3 bg-[#1A1D26] text-[#3A3D48] rounded-full">
                      <MessageSquare className="w-6 h-6 stroke-[1.5]" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-[#FAFAF8]">No reviews yet</p>
                      <p className="text-xs text-[#3A3D48] mt-1 max-w-xs leading-relaxed">Be the first to experience this stay and share your review with the StayHive community.</p>
                    </div>
                  </div>
              )}
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#0F1117] border border-[#1A1D26] rounded-2xl p-6 sticky top-24 space-y-5">
              ...
              <div className="border border-[#2A2D38] rounded-xl overflow-hidden divide-y divide-[#2A2D38]">
                <div className="grid grid-cols-2 divide-x divide-[#2A2D38]">
                  <div className="p-3 bg-[#1A1D26] hover:bg-[#2A2D38] transition-colors">
                    <label className="block text-[9px] font-extrabold text-[#3A3D48] uppercase tracking-wider mb-1">Check-in</label>
                    <input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)}
                           className="text-xs font-bold text-[#FAFAF8] bg-transparent focus:outline-none w-full cursor-pointer" />
                  </div>
                  <div className="p-3 bg-[#1A1D26] hover:bg-[#2A2D38] transition-colors">
                    <label className="block text-[9px] font-extrabold text-[#3A3D48] uppercase tracking-wider mb-1">Check-out</label>
                    <input type="date" value={checkOut} onChange={e => setCheckOut(e.target.value)}
                           className="text-xs font-bold text-[#FAFAF8] bg-transparent focus:outline-none w-full cursor-pointer" />
                  </div>
                </div>
                <div className="p-3 bg-[#1A1D26] hover:bg-[#2A2D38] transition-colors">
                  <label className="block text-[9px] font-extrabold text-[#3A3D48] uppercase tracking-wider mb-1">Guests</label>
              ...
              <button
                  onClick={() => toast.success('Secure gateway loaded', {
                ...
                {[
                  ...
                  ['StayHive service fee', `$${serviceFee}`],
                    ...
                    <div key={label} className="flex justify-between">
                      <span className="underline decoration-dotted cursor-pointer hover:text-[#FAFAF8] transition-colors">{label}</span>
                      <span>{val}</span>
                    </div>
                ))}
              ...
              <div className="p-3.5 bg-[#1A2A0A] border border-[#2A4010] rounded-xl flex items-start gap-2.5">
                ...
                <div>
                  <span className="text-[10px] font-extrabold text-[#A8D44A] uppercase tracking-wider block mb-0.5">StayHive Protection</span>
