...
const navItems = [
    ...
    {name: 'My Properties', path: '/my-properties', active: true},
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              Stay<span className="text-[#C8FB4C]">Hive</span>
                    ...
                    <div className="hidden lg:flex items-center gap-4 flex-shrink-0">
                        ...
                        <Link
                            to="/become-a-host"
                            onClick={handleHostClick} // Add this handler
                            className="relative group flex items-center gap-1.5 text-[13px] font-semibold text-[#C8FB4C] bg-[#1A2A0A]/40 border border-[#C8FB4C]/30 hover:border-[#C8FB4C] px-3.5 py-1.5 rounded-lg transition-all duration-300 shadow-[0_0_15px_rgba(200,251,76,0.05)] hover:shadow-[0_0_20px_rgba(200,251,76,0.15)]"
                        >
                            <Sparkles className="w-3.5 h-3.5 text-[#C8FB4C] animate-pulse"/>
                            <span>Become a Host</span>
                        </Link>

                        <div className="w-px h-6 bg-[#2A2D38]"/>

                        {user ? (
                            <div className="flex items-center gap-2">
                                <Link
                                    to="/my-properties"
                                    className="px-4 py-2 bg-[#1A1D26] hover:bg-[#2A2D38] border border-[#2A2D38] hover:border-[#C8FB4C]/50 text-[#FAFAF8] hover:text-[#C8FB4C] text-[13px] font-bold rounded-lg transition-colors flex items-center justify-center"
                                >
                                    My Properties
                                </Link>
                                <button
                                    onClick={logout}
                                    className="px-4 py-2 bg-[#1A1D26] text-[#FAFAF8] text-[13px] font-bold rounded-lg hover:bg-[#2A2D38] transition-colors cursor-pointer"
                                >
                                    Logout
                                </button>
                            </div>
                    ...
                    <div className="flex flex-col gap-2 mt-4">
                        <Link
                            ...
                            onClick={(e) => {
                                ...
                                handleHostClick(e); // Check auth
                            }}
                            className="w-full py-2.5 flex items-center justify-center gap-2 bg-[#1A2A0A] border border-[#C8FB4C]/40 text-[#C8FB4C] text-sm font-semibold rounded-lg text-center"
                        >
                        ...
                        {user ? (
                            <div className="flex flex-col gap-2">
                                <Link
                                    to="/my-properties"
                                    onClick={() => setIsOpen(false)}
                                    className="w-full py-2.5 bg-[#1A1D26] hover:bg-[#2A2D38] border border-[#2A2D38] hover:border-[#C8FB4C]/50 text-[#FAFAF8] hover:text-[#C8FB4C] text-sm font-semibold rounded-lg text-center transition-colors block"
                                >
                                    My Properties
                                </Link>
                                <button
                                    ...
                                    className="w-full py-2.5 bg-[#1A1D26] text-[#FAFAF8] text-sm font-bold rounded-lg hover:bg-[#2A2D38] transition-colors cursor-pointer"
                                >
                                    Logout
                                </button>
                            </div>
